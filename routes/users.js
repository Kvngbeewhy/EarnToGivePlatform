...
const { formatter } = require("../service/commonFunctions");
...
router.get("/mothon",  async (req, res) => {
  let criteria = {};
  var skipVal, limitVal;
  if (isNaN(parseInt(req.query.offset))) skipVal = 0;
  else skipVal = parseInt(req.query.offset);
  ...
  res.send({ statusCode: 200, message: USER_CONSTANTS.GET_UPDATES, data: { userList } });
});
